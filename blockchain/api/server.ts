...
app.post('/api/aggregate', (req, res) => {
    try {
        ...
        const decryptedValues = aggregateData(encryptedData, aggregationType);
        console.log(`Decrypted ${decryptedValues.length} values`);

        res.json({
            success: true,
            requestId,
            decrypted: decryptedValues,
            message: `Successfully decrypted ${encryptedData.length} records`
        });
    } catch (error: any) {
